function [parameters, states, chi_squares, number_iterations, execution_time] = ...
    gpufit_double_lorentzian_relative_constrained(data_X, data_Y, freq_begin, freq_end, weights, single_fit, constraints)
%% Gpufit call
...
model_id = ModelID.DOUBLE_LORENTZIAN_1D_REL;
number_parameters = 7;
...
for i=1:number_fits
    ...
    % Lorentzian #1
    % Amplitude_1
    initial_parameters(1, i) = single_fit.parameters(1,i) / 2;
    % x_0_1 : start at the left
    initial_parameters(2, i) = single_fit.parameters(2,i) ;
    % gamma_squarred_1
    initial_parameters(3, i) = single_fit.parameters(3,i)^2 / 2;

    % Lorentian #2 
    % Amplitude_2
    initial_parameters(4, i) = single_fit.parameters(1,i) / 2 ;
    % x_0_2 : start at the right
    initial_parameters(5, i) = -0.1;
    % gamma_squarred_2
    initial_parameters(6, i) = single_fit.parameters(3,i)^2 / 2;
   
    % offset
    initial_parameters(7, i) = 0.7;
end

% Constrains
% If no constraints are passed, we create our own
...
%Converting L2's shift to a absolute position
for i=1:size(number_iterations, 2)
    parameters(5,i) = parameters(5,i) + parameters(2,i);
end

 %Making sure L1 is always the lower peak
 for i=1:size(number_iterations, 2)
